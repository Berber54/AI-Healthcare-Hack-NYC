...
| B3 | 2026-07-11 | Red-flag detector's `swelling_with_fever` combo matched on the literal substring "fever" even inside a negated phrase (e.g. "swollen but no fever"), misfiring `DENTAL_EMERGENCY` on a routine turn | Added a negation guard (`_present_and_not_negated`) that skips a symptom match if a negation word appears immediately before it |
| B4 | 2026-07-11 | Red-flag detector's `stroke_signs` pattern only matched literal phrases "face...droop" (trailing `\b` broke on "drooping") and "slurred speech" (failed on reversed word order "speech is slurred") — real caller phrasing was silently missed | Rewrote pattern to match either word order and verb inflection (`droop\w*`, `slurr\w*`) instead of one literal phrase |
