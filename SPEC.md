...
1. A red-flag detector runs on every user turn. It's keyword/regex-based, not model-dependent. *(depends on I.claude)*
   - **⚠️ Architectural limitation, same root cause as the Invariant 8 note below:** ElevenLabs' native Twilio integration owns the live turn loop, so our backend has no true pre-turn hook to run `check_red_flag` on every turn independent of the LLM. The mitigation wired in `backend/agent/webhook.py`/`triage_prompt.py`: `check_red_flag` is registered as a tool and the prompt's "rule zero" instructs the agent to call it, with the caller's exact words, before anything else, every turn. The *decision and the escalation actions* (script, booking, transfer, SMS) are 100% regex-driven and non-negotiable once that tool fires — that part of Invariant 1/2 is fully deterministic. What is **not** fully guaranteed is the *invocation* itself: an LLM could in principle skip calling the tool on a given turn. Closing that gap for real would mean routing voice through a custom-LLM webhook (our own turn loop) instead of ElevenLabs' native integration — out of scope for this build window, same tradeoff already accepted for Invariant 8.
...
| T4 | Done | Red-flag detector built and tested in isolation (`app/safety/red_flags.py`, 20 cases in `tests/test_red_flags.py`) before wiring into the main flow | Invariants 1, 2 |
| T5 | Done | Emergency escalation path (`app/safety/escalation.py`) wired into `backend/agent/webhook.py` via a `check_red_flag` tool + "rule zero" in the triage prompt; `escalate_to_oncall`/`transfer_call` also exposed directly per I.tools. Verified via `backend/tests/test_safety_wiring.py` (7 tests) and a live TestClient smoke test through `/tools/check_red_flag`. See Invariant 1 note above for the one known gap (LLM-initiated, not a hard server gate) | Invariants 3, 4 |
...
| B6 | 2026-07-11 | T4/T5's `app/safety/*` module (built and isolation-tested standalone) couldn't actually be imported from the running server — `backend/agent/webhook.py` needs `from app.safety import ...`, but `backend/app/` (no `__init__.py`, a namespace package) and the repo-root `app/` (has `__init__.py`, a regular package) are both named `app`; whichever gets imported first wins the name outright and the other becomes unreachable. Escalation was fully built but silently dead code from the live call's perspective — `escalate_to_oncall`/`transfer_call` were never registered as tool handlers and `check_red_flag` was never called on any turn | `backend/agent/webhook.py` now extends `backend/app`'s namespace-package `__path__` to also include the repo-root `app/` dir before importing `app.safety.*`, merging the two without moving either directory (CLAUDE.md requires `app/safety/` stay separate from `backend/app/`). Also added `check_red_flag`/`escalate_to_oncall`/`transfer_call` tool handlers, a `check_red_flag` tool + "rule zero" prompt instruction, and `ONCALL_PHONE_NUMBER` to `.env.template` (was missing entirely, and `escalate_to_oncall` also referenced a nonexistent `TWILIO_FROM_NUMBER` — pointed it at the existing `TWILIO_PHONE_NUMBER` instead). Verified with `backend/tests/test_safety_wiring.py` and a live `TestClient` call through `/tools/check_red_flag` |
