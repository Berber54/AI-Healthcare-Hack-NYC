...
| T3 | Prompt/tools/personalization wired to live Supabase data layer (`agent/webhook.py`); barge-in confirmed via ElevenLabs API (`disable_first_message_interruptions=False`, no `interruption_ignore_terms`) — Invariant 10 satisfied, no custom build needed. Pending real-call verification only | Triage prompt + 4 non-emergency buckets (routine/urgent/insurance/booking) working end to end | Invariant 5, I.claude |
| T4 | Done | Red-flag detector built and tested in isolation (`backend/app/safety/red_flags.py`, 27 tests passing, see B5/B6) before wiring into the main flow | Invariants 1, 2 |
| T5 | Wired into the live call flow — `agent/webhook.py`'s new `/elevenlabs/v1/chat/completions` custom-LLM endpoint runs `check_red_flag` on every turn before Claude sees it, routing red flags to `handle_red_flag_turn` instead (Invariant 2). ElevenLabs agent config switched to `llm: custom-llm` pointed at this endpoint via the API (see B11); pending real-call verification | Emergency escalation path wired into the call flow (dental emergency + non-dental red flag) | Invariants 3, 4 |
| T6 | Done | Post-call SMS + call logging — `call_logs` table (`backend/db/schema.sql`), stateless run_id-keyed `backend/app/logger.py` (I.logger), `backend/app/sms.py` (I.sms) triggered by the `/call-status` webhook, `GET /calls/{run_id}` JSON log view (Invariant 11), and `backend/app/error_recovery.py` `retry_once` fallback wrapping data/logger reads (Invariant 12, Person B's slice). Verified 21/22 in `backend/tests/`; the one live-DB logger round-trip passes once `call_logs` is provisioned + the PostgREST cache is reloaded (see B4). Logger is now also wired into the live call flow, see B10 | Invariant 6, I.sms |
...
| B4 | 2026-07-11 | After creating the T6 `call_logs` table, the REST client kept raising `PGRST205: Could not find the table 'public.call_logs' in the schema cache` — PostgREST caches the DB schema and hadn't picked up the new table; the OpenAPI introspection at `GET /rest/v1/` still listed only the three T2 tables | Run `notify pgrst, 'reload schema';` in the SQL editor (or click "Reload schema" in the dashboard) after any DDL. Documented the provisioning + reload step in CLAUDE.md. Note: if the reload doesn't take, confirm the SQL editor and the app's `SUPABASE_KEY` point at the same database (a Supabase preview branch keeps the same dashboard project-ref in the URL but is a separate DB) |
| B5 | 2026-07-11 | Red-flag detector's `swelling_with_fever` combo matched on the literal substring "fever" even inside a negated phrase (e.g. "swollen but no fever"), misfiring `DENTAL_EMERGENCY` on a routine turn | Added a negation guard (`_present_and_not_negated`) that skips a symptom match if a negation word appears immediately before it |
| B6 | 2026-07-11 | Red-flag detector's `stroke_signs` pattern only matched literal phrases "face...droop" (trailing `\b` broke on "drooping") and "slurred speech" (failed on reversed word order "speech is slurred") — real caller phrasing was silently missed | Rewrote pattern to match either word order and verb inflection (`droop\w*`, `slurr\w*`) instead of one literal phrase |
| B7 | 2026-07-11 | Person D's `app/safety/` (red-flag detector + escalation, T4/T5) lived at repo root, while Person B/C's data layer lives at `backend/app/`. `backend/app/` has an `__init__.py` (regular package), so it can't namespace-merge with root `app/` — `app.safety.X` and `app.data` couldn't both import in the same process, blocking T5's wiring into the live call flow | Relocated `app/safety/` (and its tests) into `backend/app/safety/`, one `app` package for the whole backend — same fix pattern as commit 407f9de's `agent/` relocation |
| B8 | 2026-07-11 | After the B7 move, `backend/tests/test_red_flags.py`/`test_escalation.py` inherited `conftest.py`'s `autouse=True` Supabase-skip fixture and got force-skipped even though they never touch Supabase | Changed the fixture to only skip tests marked `@pytest.mark.requires_supabase`; applied that marker to the three data-layer test files (`test_data_access.py`, `test_schema.py`, `test_seed_data.py`) |
| B9 | 2026-07-11 | T6's `requirements.txt` bumped `httpx` to `0.28.1`, but `supabase==2.10.0` pins `httpx<0.28` — installing the pinned versions together is unsatisfiable, so `pip install -r requirements.txt` fails | Reverted `httpx` to `0.27.2` in `backend/requirements.txt` |
| B10 | 2026-07-11 | T6's `backend/app/logger.py` (`start_call`/`log_turn`/`log_classification`/`log_tool_call`/`log_escalation`) was built and unit-tested in isolation but never called from `backend/agent/webhook.py` — only `end_call`/`mark_sms_sent` fired, from `/call-status`. A real call would produce an empty `call_logs` row (no transcript, no tool_calls, no escalations) until hangup, violating Invariants 6/11 despite T6 being marked Done | Wired `start_call` into `/elevenlabs/personalize`, `log_turn`/`log_classification`/`log_escalation` into `/elevenlabs/v1/chat/completions`, and `log_tool_call` into `/tools/{tool_name}`, keyed off `call_sid` extracted the same way `_call_context_from_request` already did |
| B11 | 2026-07-11 | The live ElevenLabs agent (`arya-healthcare`) was never actually pointed at our webhooks: `conversation_config.agent.prompt.llm` was still the default `gemini-2.5-flash` (not our custom-LLM endpoint), and `platform_settings.overrides.enable_conversation_initiation_client_data_from_webhook` was `false` with no webhook URL set — so T3/T4/T5's code (triage prompt, red-flag detector, escalation, personalization, B10's logger wiring) never ran on a real call despite being wired and tested locally | PATCHed the agent via the ElevenLabs API: set `prompt.llm` to `custom-llm` with `prompt.custom_llm.url` = `{PUBLIC_BASE_URL}/elevenlabs/v1/chat/completions`; enabled `enable_conversation_initiation_client_data_from_webhook` and set `workspace_overrides.conversation_initiation_client_data_webhook.url` = `{PUBLIC_BASE_URL}/elevenlabs/personalize`. Fragile: `PUBLIC_BASE_URL` is a free-tier ngrok subdomain — restarting ngrok changes it and both settings need re-patching |
