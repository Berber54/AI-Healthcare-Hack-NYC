...
class DailyContextService:
    ...
    async def upsert(self, user_id: str, plan: DailyPlanData) -> None:
        ...
        plan_date = datetime.now(tz=ET).date().isoformat()
        ...
        try:
            # upsert: insert or update on (user_id, plan_date) conflict
            await self.db.table("daily_context").upsert(
                row,
                on_conflict="user_id,plan_date",
                # increment refresh_count on update
                count="exact",
            ).execute()
            logger.info(f"daily_context upserted for user={user_id} date={plan_date}")
    ...
    async def fetch(self, user_id: str, plan_date: date | None = None) -> dict | None:
        ...
        target = (plan_date or datetime.now(tz=ET).date()).isoformat()
        try:
    ...
    async def wipe_stale(self) -> int:
        """Delete all rows where plan_date is before today ET. Returns count deleted."""
